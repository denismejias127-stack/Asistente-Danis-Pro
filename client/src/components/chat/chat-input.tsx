...
export function ChatInput({
    ...
    for (const file of files) {
      try {
        if (
          ...
          [".txt",".md",".csv",".json",".xml",".html",".js",".ts",".py",".log"].some(ext => file.name.endsWith(ext))
        ) {
  ...
  const handleVoiceToggle = () => {
    ...
    if (!SpeechRec) {
      toast({ title: "No disponible", description: "Tu navegador no soporta reconocimiento de voz. Prueba Chrome.", variant: "destructive" });
      return;
    }
    ...
    recognition.onerror = (event: any) => {
      ...
      if (event.error === "no-speech") {
        ...
        toast({ title: "Error", description: "No se pudo escuchar. Intenta de nuevo.", variant: "destructive" });
      }
    };
  ...
  const handleSend = () => {
    ...
    if (hasContent && !isGenerating && !isListening) {
      ...
      onSend(finalMessage, images, false);
      setInput("");
      setImages([]);
      setAttachedFiles([]);
      if (textareaRef.current) textareaRef.current.style.height = "inherit";
  ...
  return (
    <div className="relative p-3 pb-5 w-[92%] mx-auto">
      ...
      <input ref={galleryInputRef} type="file" accept="image/jpeg,image/png,image/gif,image/webp,image/heic" multiple className="hidden" onChange={handleGalleryPick} data-testid="input-file-gallery" />
      <input ref={fileInputRef} type="file" accept=".txt,.md,.csv,.json,.xml,.html,.js,.ts,.py,.log" multiple className="hidden" onChange={handleFilePick} data-testid="input-file-docs" />
      ...
      <div
        className={`relative flex items-end w-full glass-panel rounded-[1.5rem] shadow-lg shadow-black/5 p-2 transition-all focus-within:ring-2 ${
          isListening ? "ring-2 ring-red-500/40 border-red-500/30" :
      ...
      >
        ...
        <div className="flex-shrink-0 flex items-center ml-2 mb-1 mr-1 gap-1">
          <Button
            ...
            title={isListening ? "Toca para detener" : "Hablar"}
          >
            {isListening ? (
          ...
          <Button
            ...
            disabled={(!input.trim() && !isGenerating && images.length === 0 && attachedFiles.length === 0) || isListening}
